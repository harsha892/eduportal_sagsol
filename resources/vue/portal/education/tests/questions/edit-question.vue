<template>
  <div id="editQuestion">
    <div class="card">
      <div class="card-body">
        <form id="app" @submit="updateQuestion" v-if="formData">
          <div class="form-row">
            <div class="col-3">
              <auto-complete label="topic" :inputValue="formData.topic_name" @value="getTopic"></auto-complete>
            </div>
            <div class="col-3">
              <div class="form-group">
                <label for="inputAddress">Question Type</label>
                <select class="custom-select" :value="formData.type" disabled>
          ...
          <div class="form-row">
            ...
            <div class="col-12 text-right">
              <span class="btn btn-info text-white" v-on:click="updateQuestion()">Update Question</span>
            </div>
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    updateField(event, key) {
      ...
      this.$store.commit("question/UPDATE_SINGLE_QUESTION_FIELD", payload);
    },
    getTopic(topic) {
    ...
    updateQuestion() {
      this.$store.dispatch("question/UPDATE_QUESTION", this.formData);
    }
  }
};
</script>